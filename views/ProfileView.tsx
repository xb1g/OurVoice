...
export const ProfileView: React.FC<ProfileViewProps> = ({ user, onUpdateUser, onSwitchUser }) => {
            ...
            <div className="mt-6 w-full max-w-xs">
                ...
                <div className="relative">
                    <select 
                        ...
                        className="w-full appearance-none bg-slate-50 text-slate-700 text-sm font-bold py-3 px-4 rounded-2xl border-none focus:ring-2 focus:ring-indigo-500/20 text-center cursor-pointer hover:bg-slate-100 transition-colors"
                    >
                        {MOCK_USERS.map(u => (
                        <option key={u.id} value={u.id}>view as {u.name} ({u.role})</option>
                        ))}
                    </select>
